...
export function DenominationGrid({ products, selectedProduct, onSelectProduct, stepNum }: DenominationGridProps) {
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-[hsl(var(--card))]/40 backdrop-blur-xl rounded-2xl border border-white/10 p-5 sm:p-7 shadow-2xl">
      ...
      <div className="grid grid-cols-2 gap-3 sm:gap-6 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-3">
        {products.map((product) => (
          <button
            key={product.id}
            onClick={() => onSelectProduct(product.id)}
            className={cn(
              "relative flex flex-col items-start gap-1.5 rounded-xl border p-4 sm:p-5 text-left transition-all duration-200 cursor-pointer overflow-hidden backdrop-blur-sm min-h-[72px]",
