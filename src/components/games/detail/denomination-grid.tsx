...
export function DenominationGrid({ products, selectedProduct, onSelectProduct, stepNum }: DenominationGridProps) {
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-[hsl(var(--card))]/40 backdrop-blur-xl rounded-2xl border border-white/10 p-5 sm:p-7 shadow-2xl">
      <StepBadge num={stepNum} title="Pilih Nominal" isCompleted={!!selectedProduct} />
      <div className="grid grid-cols-2 gap-3 sm:gap-6 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-3" role="radiogroup" aria-label="Pilihan Nominal">
        {products.map((product) => (
          <button
            ...
            className={cn(
              "relative flex flex-col items-start gap-1.5 rounded-xl border p-4 sm:p-5 text-left transition-all duration-200 cursor-pointer overflow-hidden backdrop-blur-sm min-h-[72px] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[hsl(var(--primary))]",
          ...
          >
            ...
            <span className="text-sm sm:text-sm font-bold text-[hsl(var(--foreground))] leading-tight mb-1 pr-4">
              {product.name}
            </span>
            <span className="text-sm font-bold text-[hsl(var(--primary))] mt-auto drop-shadow-sm">
